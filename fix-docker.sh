...
echo "3️⃣ Starting with port ${PORT:-5099}..."
...
if docker ps | grep -q "vision-dashboard"; then
    ...
    if timeout 10 curl -f http://localhost:${PORT}/api/health >/dev/null 2>&1; then
        echo "✅ Application is responding!"
    else
        echo "⏳ Application starting up... (may take up to 30 seconds)"
        echo "💡 Try manually: curl http://localhost:${PORT}/api/health"
    fi
else
